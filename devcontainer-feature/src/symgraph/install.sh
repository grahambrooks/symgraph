#!/usr/bin/env bash
#
# Devcontainer feature install for symgraph.
#
...
#   - Installs to system-wide paths (/usr/local/lib/symgraph, /usr/local/bin)
#     rather than the per-user ~/.symgraph default.
...
INSTALL_DIR="/usr/local/lib/symgraph"
...
export SYMGRAPH_VERSION="${VERSION}"
export SYMGRAPH_INSTALL_DIR="${INSTALL_DIR}"
...
ln -sf "${INSTALL_DIR}/bin/symgraph" "${BIN_DIR}/symgraph"
